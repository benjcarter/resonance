          ...
          {items.map((item) => (
            <SidebarMenuItem key={item.title}>
              <SidebarMenuButton
                ...
                tooltip={item.title}
                className="h-9 px-3 py-2 text-[13px] tracking-tight font-medium border border-transparent data-active:border-border data-active:shadow-[0px_1px_1px_0px_rgba(44,54,53,0.03),inset_0px_0px_0px_2px_white]"
              >
                {item.url ? (
                  <Link href={item.url}>
                    <item.icon />
...
export function DashboardSidebar() {
  ...
  const othersMenuItems: MenuItem[] = useMemo(
    () => [
      {
        title: "Settings",
        icon: SettingsIcon,
      ...
      {
        title: "Help & Support",
        url: "mailto:support@resonance.ai",
        icon: HeadphonesIcon,
      },
    ],
    [clerk],
